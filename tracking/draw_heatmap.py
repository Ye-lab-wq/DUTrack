...
def visualize_language_token_weights(tokens, weights, save_path, description=None,
                                     status_lines=None, title="Language token weights"):
    if isinstance(weights, torch.Tensor):
        weights = weights.detach().float().cpu().view(-1).numpy()
    else:
    ...
    with open(txt_path, 'w', encoding='utf-8') as f:
        ...
        for rank, idx in enumerate(order, start=1):
            f.write('{}\t{}\t{:.6f}\n'.format(rank, tokens[idx], float(weights[idx])))
    ...
    label_w = 210
    bar_w = 360
    canvas_h = header_h + row_h * n + pad
    canvas_w = label_w + bar_w + 150
    canvas = np.full((canvas_h, canvas_w, 3), 255, dtype=np.uint8)

    cv2.putText(canvas, title, (pad, 30), cv2.FONT_HERSHEY_SIMPLEX,
                0.75, (20, 20, 20), 2, cv2.LINE_AA)
    y = 58
    if description:
        desc = textwrap.shorten(str(description), width=92, placeholder='...')
        cv2.putText(canvas, desc, (pad, y), cv2.FONT_HERSHEY_SIMPLEX,
                    0.48, (60, 60, 60), 1, cv2.LINE_AA)
        y += 22
    if status_lines:
        status = textwrap.shorten(' | '.join(str(s) for s in status_lines), width=96, placeholder='...')
        cv2.putText(canvas, status, (pad, y), cv2.FONT_HERSHEY_SIMPLEX,
                    0.45, (80, 80, 80), 1, cv2.LINE_AA)

    max_w = float(np.max(weights)) if np.max(weights) > 0 else 1.0
    for row, idx in enumerate(order):
        token = tokens[idx]
        weight = float(weights[idx])
        y0 = header_h + row * row_h
        y_text = y0 + 21
        label = '{}. {}'.format(row + 1, token)
        cv2.putText(canvas, textwrap.shorten(label, width=28, placeholder='...'),
                    (pad, y_text), cv2.FONT_HERSHEY_SIMPLEX, 0.52,
                    (30, 30, 30), 1, cv2.LINE_AA)

        x0 = label_w
        x1 = x0 + int(round(bar_w * weight / max_w))
        color = (60, 120, 220) if row < 5 else (150, 170, 190)
        cv2.rectangle(canvas, (x0, y0 + 7), (x1, y0 + row_h - 7), color, -1)
        cv2.rectangle(canvas, (x0, y0 + 7), (x0 + bar_w, y0 + row_h - 7), (220, 220, 220), 1)
        cv2.putText(canvas, '{:.4f}'.format(weight), (x0 + bar_w + 14, y_text),
