...
class HOOTDataset(BaseDataset):
    ...
    Expected extracted layout follows the official hoot-toolkit:
        HOOT_ROOT/
            test.txt
            train.txt
            <class_name>/
    ...
    def _get_sequence_list(self):
        ...
        if not sequence_list:
            raise RuntimeError(
                'No HOOT videos found under {}. Download with hoot-toolkit using --extract, '
                'or verify the class/video/anno.json layout.'.format(self.base_path)
            )
    ...
    def _construct_sequence(self, class_name, video_id, sequence_name):
        ...
        frames = sorted(anno['frames'], key=lambda item: int(item['frame_id']))
        frame_paths = []
        boxes = []
        target_visible = []
        ...
        for frame in frames:
            ...
            aa_bb = frame.get('aa_bb', [])
            boxes.append(self._polygon_to_xywh(aa_bb))

            # Keep full-occlusion frames evaluable when a box is annotated; skip only absent/out-of-frame frames.
        ...
        seq = Sequence(sequence_name, frame_paths, self.dataset_name, np.array(boxes, dtype=np.float64).reshape(-1, 4),
                       object_class=target_class,
                       target_visible=np.array(target_visible, dtype=bool),
                       text_description=target_class)
