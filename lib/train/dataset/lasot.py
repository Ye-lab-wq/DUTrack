...
import bisect
# from lib.models.odtrack.i2d import descriptgenRefiner
...
class Lasot(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, vid_ids=None, split=None, data_fraction=None):
        ...
        # self.descriptgenRefiner = descriptgenRefiner("/home/local_data/lxh/data/blip-image-captioning-base")
    ...
    # def _get_txt_path(self, seq_id):
    #     seq_name = self.sequence_list[seq_id]
    #     class_name = seq_name.split('-')[0]
    #     vid_id = seq_name.split('-')[1]
    #
    #     return os.path.join(self.root, "lasot_train_concise", seq_name+'.txt')

    def get_language_description(self,seq_id,frame_ids):
        seq_name = self.sequence_list[seq_id]
        txt_path = os.path.join(self.root, "lasot_train_concise", seq_name+'.txt')
        result_dict = {}
        ...
        idx = bisect.bisect_left(keys, frame_ids)
        if idx == 0:
            id = keys[0]
        elif idx == len(keys):
            id = keys[-1]
        ...
        language_description = result_dict[id]
        # txt_path = self._get_txt_path(seq_id)
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        # des = self.descriptgenRefiner(frame_list[0],cls=obj_class)
